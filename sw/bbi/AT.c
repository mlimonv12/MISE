...
uint8_t AP_Mode = AP_MODE; //Modo 1: Station, 2: AP, 3: AP + Station
...
StrConexion conexion; //El identificador de conexion, lo limito a 1 caracter "0"..."9" = 0x30...0x39, inicializado a "0".
...
uint8_t get_Modo_Wifi(){
    if (AP_Mode == MODO_AP) return MODO_AP;
    else return MODO_STA;
}
...
int8_t init_servidor_esp(uint8_t modo){
    ...
    if(!comando_AT()) return ERR_NO_AT; //Si no responde al comando de test mas basico "AT", abortamos
    ...
    AP_Mode = modo;
    if (AP_Mode == 2) TxAT(sizeof(modo_AP)-1,modo_AP);
    else TxAT(sizeof(modo_Cliente)-1,modo_Cliente);
    respuesta = RxAT(50);
    //    if (pinta_Modo(linea++, respuesta) < 0) return -1; //No hace falta seguir, algo falla con el modulo wifi...

    if (AP_Mode == 2) { //Si somos AP:
        ...
        error = esperar_configuracion(20000); //con un timeout de 200x10ms = 20s
    }
    if (error < 0)  {
...
struct RxATReturn decodificar_trama(struct RxATReturn trama){
    ...
    //Si no hemos encontrado ningun c�digo AT, no sabemos que c... hemos recibido:
